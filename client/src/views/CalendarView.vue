...
const selectedEvent = ref(null); //evento seleccionado para el panel lateral

//configuracion del componente FullCalendar
const calendarOptions = ref({
  ...
  //events como funcion para que FullCalendar pase el rango de fechas visible
  events: function (info, successCallback, failureCallback) {
    let start = info.startStr.split("T")[0];
    let end = info.endStr.split("T")[0];
    let url =
      rutaApi +
      "?entity=calendar&user_id=" +
      userStore.user.id +
      "&start=" +
      start +
      "&end=" +
    ...
    fetch(url)
      ...
      .then(function (data) {
        //si la respuesta es un array la pasamos, si no mostramos error
        if (Array.isArray(data)) {
          successCallback(data);
        } else {
          errorMessage.value = data.message || "Error loading calendar";
          failureCallback();
        }
      })
      .catch(function () {
...
//funcion que se dispara al hacer clic en un evento
...
//funcion que calcula cuanto tiempo queda hasta una fecha
function getCountdown(dateStr) {
  ...
  //calculamos diferencias en distintas unidades de tiempo
...
//funcion auxiliar que devuelve el texto del tipo de evento
...
//funcion auxiliar que devuelve el icono del tipo de evento
...
<template>
  <div class="calendar-container">
    <!-- cabecera con titulo y leyenda de colores -->
    <div class="d-flex align-items-end justify-content-between mb-4 fade-up">
      <h1 class="page-title"><em>plan your</em> Calendar</h1>
    ...
    <div class="row g-4">
      ...
      <!-- panel lateral con detalles del evento seleccionado -->
      <div v-if="selectedEvent" class="col-12 col-lg-4 fade-up">
        <div class="event-panel">
          <!-- cabecera del panel con el color del evento como borde -->
          <div
          ...
          <div class="event-panel-body">
            ...
            <!-- cuenta regresiva solo para tareas pendientes con fecha de fin -->
            <div
              v-if="
            ...
            >
              ...
              <div>
                ...
                <div class="detail-value">
                  <span
                    v-if="getCountdown(selectedEvent.end).expired"
                    class="countdown-expired"
                  >
                    <i class="bi bi-exclamation-triangle me-1"></i>Overdue
                  </span>
                  <span
                    v-else-if="getCountdown(selectedEvent.end).mode === 'hours'"
                  ...
                  <span
                    v-else-if="getCountdown(selectedEvent.end).mode === 'days'"
                  ...
                  <span
                    v-else-if="getCountdown(selectedEvent.end).mode === 'weeks'"
                    class="countdown-weeks"
                  >
            ...
            <!-- frecuencia si es habito o rutina -->
              ...
              <div>
                ...
                <div class="detail-value">
                  <span
                    class="bdg"
                    :class="
                      selectedEvent.frecuency === 'daily'
                        ? 'bdg-daily'
                        : selectedEvent.frecuency === 'weekly'
                          ? 'bdg-weekly'
                          : 'bdg-monthly'
                    "
                  >
            ...
            <div class="event-detail-row">
              <i
                class="bi detail-icon"
                :class="
                  selectedEvent.done ? 'bi-check-circle-fill' : 'bi-circle'
                "
              ></i>
              <div>
                ...
                <div class="detail-value">
                  <span
                    class="bdg"
                    :class="selectedEvent.done ? 'bdg-done' : 'bdg-daily'"
                  >
...
/* overrides de estilos de fullcalendar con :deep */
:deep(.fc) {
...
/* dia de hoy resaltado con fondo y circulo */
:deep(.fc-day-today) {
...
/* cabecera del panel con borde izquierdo del color del evento */
.event-panel-header {
...
/* colores de la cuenta regresiva segun urgencia */
.countdown-expired {
