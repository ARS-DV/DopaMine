...
function toggleRole(user) {
  ...
  let label   = newRole === 'admin' ? 'promote to admin' : 'remove admin role from'

  let confirmed = confirm('Are you sure you want to ' + label + ' ' + user.nickName + '?')
  if (!confirmed) return
...
function deleteUser(user) {
  let confirmed = confirm('DELETE account of ' + user.nickName + '? This will also remove all their habits, tasks and routines. This cannot be undone.')
  if (!confirmed) return
  ...
  .then(function(data) {
    if (data.status === 'success') {
      users.value = users.value.filter(function(u) { return u.id !== user.id })
    } else {
      error.value = data.message
    }
  })
...
// estadísticas globales calculadas desde la lista
const totalUsers   = computed(function() { return users.value.length })
const totalAdmins  = computed(function() { return users.value.filter(function(u) { return u.role === 'admin' }).length })
const totalHabits  = computed(function() { return users.value.reduce(function(acc, u) { return acc + (u.habits_count || 0) }, 0) })
const totalTasks   = computed(function() { return users.value.reduce(function(acc, u) { return acc + (u.tasks_count || 0) }, 0) })

// lista filtrada por búsqueda y rol
const filteredUsers = computed(function() {
  let list = users.value

  if (filterRole.value === 'admin') {
    list = list.filter(function(u) { return u.role === 'admin' })
  } else if (filterRole.value === 'user') {
    list = list.filter(function(u) { return u.role === 'user' })
  }

  if (searchQuery.value.trim() !== '') {
    let q = searchQuery.value.toLowerCase()
    list  = list.filter(function(u) { return u.nickName.toLowerCase().includes(q) })
  }
...
<template>
  <div class="admin-container">
    ...
    <template v-else>
      ...
      <!-- STATS STRIP -->
      <div class="row g-3 mb-4 fade-up delay-1" aria-label="Platform statistics">
      ...
      <div class="admin-toolbar mb-4 fade-up delay-2">
        <div class="search-wrapper">
          ...
          <label for="user-search" class="visually-hidden">Search users by nickname</label>
          <input
            id="user-search"
            v-model="searchQuery"
            type="search"
        ...
        <div class="d-flex gap-2" role="group" aria-label="Filter by role">
          <button class="filter-tab" :class="filterRole === 'all'   ? 'active' : ''" @click="filterRole = 'all'">All ({{ totalUsers }})</button>
          <button class="filter-tab" :class="filterRole === 'user'  ? 'active' : ''" @click="filterRole = 'user'">Users</button>
          <button class="filter-tab" :class="filterRole === 'admin' ? 'active' : ''" @click="filterRole = 'admin'">Admins</button>
        </div>
      </div>
      ...
      <!-- LISTA DE USUARIOS -->
      <div v-else class="d-flex flex-column gap-3" role="list" aria-label="User list">
        <article
        ...
        >
          <div class="user-card-body">
            ...
            <div class="d-flex align-items-center gap-3 flex-grow-1">
              ...
              <div class="flex-grow-1" style="min-width:0">
                <div class="d-flex align-items-center flex-wrap gap-2 mb-1">
                  ...
                  >
                    <i class="bi me-1" :class="user.role === 'admin' ? 'bi-shield-check' : 'bi-person'" aria-hidden="true"></i>
                    {{ user.role }}
                  </span>
                  <!-- Eres tú -->
                  <span v-if="user.id === userStore.user.id" class="bdg bdg-done">
            ...
            <div v-if="user.id !== userStore.user.id" class="d-flex gap-2 flex-shrink-0">
              <button
                class="btn-dopamine btn-dopamine-ghost admin-action-btn"
                :aria-label="(user.role === 'admin' ? 'Remove admin role from ' : 'Promote to admin: ') + user.nickName"
                :title="user.role === 'admin' ? 'Remove admin' : 'Make admin'"
                @click="toggleRole(user)"
              >
                <i
                  class="bi"
                  :class="user.role === 'admin' ? 'bi-person-dash' : 'bi-shield-plus'"
                  aria-hidden="true"
                ></i>
              </button>
              <button
            ...
            <!-- Indicador visual de que eres tú (sin botones) -->
...
@media (max-width: 768px) {
  .admin-container { padding: 1.5rem 1rem 4rem; }
}

/* BADGE ADMIN EN CABECERA */
.admin-badge {
  font-family: 'Atkinson Hyperlegible', sans-serif;
  font-size: 0.82rem;
  font-weight: 700;
...
.user-card:hover { box-shadow: 0 4px 16px rgba(92, 51, 23, 0.11); }

.card-admin { border-left-color: var(--btn-info); background: var(--state-info-bg); }
.card-user  { border-left-color: var(--vanilla-mid); }
...
.avatar-admin { background: var(--btn-info); color: #fff; }
.avatar-user  { background: var(--vanilla-mid); color: var(--cinnamon-dark); }
...
.meta-sep { color: var(--vanilla-mid); }
